...
- **`send_sequence.py`**: for every lead still `active`, checks whether their
  ...
  Sends at most one email per run, only during business hours, so volume
  comes from how often the workflow fires rather than a batch cap.
- **`.github/workflows/sequence.yml`**: runs both scripts on an hourly
  schedule during business hours and commits the updated `data/leads.json`
  back to the repo, so state (who's on which step, who replied) persists
...
- **Reply/bounce detection is simpler.** It's a keyword-based IMAP scan, not a
  polished inbox, so check in occasionally that it's not misclassifying anything.
